...
class AINAILSYSApp:
    ...
    def setup_ui(self):
        ...
        tk.Entry(left, textvariable=self.patient_name,
                 font=("Arial", 11), relief=tk.SUNKEN, bd=2
                 ).pack(fill=tk.X, ipady=5, pady=(0, 8))
        ...
        tk.Entry(left, textvariable=self.patient_age,
        ...
        tk.Label(right, text="* All fields required",
                 font=("Arial", 8),
                 bg=COLOR_LIGHTEST, fg=COLOR_MID).pack(anchor=tk.W)
        ...
        # Trace text fields for live validation
        ...
        # Bottom button row — NEW SCAN + SAVE side by side
    ...
    def _draw_logo(self, canvas, cx, cy, r):
        """Canvas-drawn logo for homepage"""
        canvas.create_oval(cx-r, cy-r, cx+r, cy+r,
                           fill=COLOR_PALE_BLUE, outline=COLOR_BLUE, width=3)
        nw, nh = 28, 36
        nx, ny = cx - nw//2, cy - nh//2 + 2
        canvas.create_arc(nx, ny, nx+nw, ny+nh,
                          start=0, extent=180,
                          fill=COLOR_BLUE, outline=COLOR_BLUE)
        canvas.create_rectangle(nx, cy, nx+nw, ny+nh,
                                 fill=COLOR_LIGHT, outline=COLOR_LIGHT)
        cs = 9
        canvas.create_rectangle(cx-2, cy+10, cx+2, cy+10+cs,
                                 fill=COLOR_SUCCESS, outline="")
        canvas.create_rectangle(cx-cs//2, cy+12, cx+cs//2, cy+16,
                                 fill=COLOR_SUCCESS, outline="")
